...
- A free trial can store its end date and stable post-trial price. Multi-phase introductory pricing (for example A$5/month for three months, then A$15/month) is not yet represented as a price schedule.
- Reminder delivery is in-app only. Email and push delivery are not configured.
- Deleting Subtrack app data does not delete the external Supabase Auth identity; the account screen discloses this explicitly.
- Gmail's restricted read-only scope must complete Google verification before a public launch.
